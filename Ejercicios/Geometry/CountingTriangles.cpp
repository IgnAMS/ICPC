...
bool int_seg(PP a, PP b){
    return (inter(a.ff, a.ss, b.ff, b.ss).size() >= 1);
}
...
int main(){
    ...
    while(cin>>n and n){
        ...
        rep(i, n) repx(j, i+1, n) repx(k, j+1, n){
            if(int_seg(Points[i], Points[j]) and int_seg(Points[i], Points[k]) and int_seg(Points[j], Points[k])) ans += 1;
        }
        cout<<ans<<'\n';
    }
