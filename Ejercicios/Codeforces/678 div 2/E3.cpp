...
int find(int v, int tl, int tr, int k) {
    // cout<<"explorando "<<tl<<' '<<tr<<' '<<t[v]<<' '<<k<<'\n';
    if(tl == tr) return tl; // retorna el tl!!!!!, el primero que no ha encontrado! 
    int tm = (tl + tr) >> 1;
    if(t[v << 1] < k){
...
int main(){
    ...
    for(int i = 1; i <= M; i++)if(last[i] && last[i] != n){
        cout<<find(1, 1, M, last[i] + 1)<<'\n';
        mp[find(1, 1, M, last[i] + 1)] = 1;
    }
    
    cout<<find(1, 1, M, 1)<<'\n'; // buscame en todo el rango!
    mp[find(1, 1, M, 1)] = 1;
